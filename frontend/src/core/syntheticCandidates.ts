...
function parseChordSymbol(symbol: string): Chord | null {
  ...
  if (symbol.length >= 2) {
    ...
    if (['A#', 'C#', 'D#', 'F#', 'G#', 'Bb', 'Db', 'Eb', 'Gb', 'Ab'].includes(twoChar)) {
...
function inferRomanNumeral(
  ...
  const scaleIntervals = scaleType === 'major' ? MAJOR_SCALE_INTERVALS : MINOR_SCALE_INTERVALS
  const romanNumerals = scaleType === 'major' ? MAJOR_ROMAN_NUMERALS : MINOR_ROMAN_NUMERALS
  ...
  if (exactDegreeIndex !== -1) {
    ...
    const isMinorQuality = ['minor', 'minor7', 'diminished', 'diminished7', 'half_diminished7'].includes(chordQuality)
    ...
    return isMinorQuality ? standardRoman.toLowerCase() : standardRoman.toUpperCase()
  }
  
  // Chromatic note - use accidental-modified roman numeral
  const chromaticMap = scaleType === 'major' ? CHROMATIC_ROMAN_MAJOR : CHROMATIC_ROMAN_MINOR
...
export function createSyntheticCandidates(
  ...
  for (const stat of statisticalRecs) {
    ...
    const harmonicFunction = inferHarmonicFunction(chord.root, map.key, map.scaleType)
    
    // Infer roman numeral from interval analysis
    const romanNumeral = inferRomanNumeral(chord.root, chord.quality, map.key, map.scaleType)
    ...
    const harmonicDistance = calculateHarmonicDistance(chord, map.key, map.scaleType)
    const colorClass = classifyColor(tempNode, map.key, map.scaleType)
    
    // Determine category from color classification
    const category = 
      colorClass === 'diatonic' ? 'diatonic' :
      colorClass === 'secondary_dominant' ? 'secondary-dominant' :
      colorClass === 'diminished_passing' ? 'diminished-passing' : 'borrowed'
