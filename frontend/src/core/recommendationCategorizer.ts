import type { ScoredCandidate, CategorizedRecommendations, ProgressionMap } from '@/schemas'
...
export function categorizeRecommendations(
  ...
  for (const candidate of candidates) {
    ...
    if (
      ...
      (breakdown.colorClass === 'diatonic' && breakdown.transitionStrength > 0.4)
    ) {
    ...
    if (
      (breakdown.colorClass !== 'diatonic' && breakdown.harmonicDistance >= 1.0) ||
    ...
    if (
      ...
      (node.romanNumeral === 'I' || node.romanNumeral === 'i')
    ) {
  ...
  categories.tension_building.sort((a, b) => b.breakdown.total - a.breakdown.total)
