...
export function isBorrowed(
  ...
  const parallelScale = getScalePattern(
 ...
 */
export function isDiminishedPassing(node: ProgressionNode, key: Note): boolean {
  // Check if it's a diminished 7th chord
  if (
    node.chord.quality !== 'diminished7' &&
    node.chord.quality !== 'diminished'
  ) {
    return false
  }
