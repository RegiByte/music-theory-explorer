...
export const HarmonicEdge = memo(
  ({
    ...
    const functionName =
      data.harmonicMovement === 'to-tonic'
        ? t('harmonicEdge.resolution')
        : data.harmonicMovement === 'to-dominant'
          ? t('harmonicEdge.tension')
      ...
      <>
        ...
        <EdgeLabelRenderer>
          ...
          >
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2 mb-2">
                    <Badge
                      ...
                      style={{
                        backgroundColor: getColorClassColor(
                          breakdown.colorClass,
                        ),
                      }}
                    >
                  ...
                  <div className="flex items-center gap-2 text-xs">
                    ...
                    <span className="text-gray-500">
                      {breakdown.harmonicDistance < 0.5
                        ? t('harmonicEdge.veryClose')
                        : breakdown.harmonicDistance < 1.5
                          ? t('harmonicEdge.close')
                          : breakdown.harmonicDistance < 2.5
                            ? t('harmonicEdge.moderate')
                            : t('harmonicEdge.far')}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-2 text-xs">
                    <span className="text-gray-600 font-medium w-20">
                      {t('harmonicEdge.tensionLabel')}
                    </span>
                    <div className="flex items-center gap-1.5">
                      <div
                        className="w-2 h-2 rounded-full"
                        style={{
                          backgroundColor: getTensionColor(
                            breakdown.tensionLevel,
                          ),
                        }}
                      />
                      ...
                      {breakdown.tensionDelta !== undefined && (
                        <span className="text-gray-500">
                          {breakdown.tensionDelta > 0.1
                            ? t('harmonicEdge.building')
                            : breakdown.tensionDelta < -0.1
                              ? t('harmonicEdge.resolving')
                              : t('harmonicEdge.stable')}
                        </span>
                      )}
                    </div>
                  </div>
            ...
            {isHovered &&
              !breakdown &&
              data.matchedProgressions &&
              data.matchedProgressions.length > 0 && (
